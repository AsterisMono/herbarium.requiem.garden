---
title: 使用 OpenWRT 搭建四路 WAN 负载均衡网关
editUrl: false
slug: openwrt-mwan3
...
使用官方 OpenWRT，x86\_64 构建。下载地址在 [这里](https://downloads.openwrt.org/releases/24.10.0/targets/x86/64/)！
...
接口添加好后，为每个接口创建一个 Member，分别命名为 wan1\_m1\~wan4\_m1。详细设置中 `metric` 是连接优先级，数值越低，优先级越高；`weight` 是分流权重，推荐根据不同线路的带宽比例来配置。由于实验室四路内网线都是 100M，我这里直接选择了默认值（均为 1）。
